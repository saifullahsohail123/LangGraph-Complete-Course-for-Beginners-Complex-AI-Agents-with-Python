...
def guess_node(state: GameState) -> GameState:
    ...
    while True:
        try:
            # Get user input
            user_guess = int(input(f"\nEnter a number between {state['lower_bound']} and {state['upper_bound']}: "))
            
            # Check if in valid range
            if user_guess < state["lower_bound"] or user_guess > state["upper_bound"]:
                print(f"❌ Invalid! The number must be between {state['lower_bound']} and {state['upper_bound']}")
                continue
            
            # Check if already guessed
            if user_guess in state["guesses"]:
                print(f"❌ You already guessed {user_guess}! Try a different number.")
                continue
            
            # Valid guess - add to list and increment attempts
            state["guesses"].append(user_guess)
            state["attempts"] += 1
            print(f"✓ Attempt {state['attempts']}: You guessed {user_guess}")
            break
